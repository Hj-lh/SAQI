...
class NavLog(Enum):
    ...
    WATER_HANDOFF_STARTED = "Auto: centered YOLO plant id=%s area=%.2f reached handoff threshold %.2f; ultrasonic owns final approach for %.1fs"
    WATER_HANDOFF_APPROACH = "Auto: ultrasonic handoff front=%.1f cm (water at <=%d cm); forward speed=%.2f for %.2fs"
    WATER_HANDOFF_READY = "Auto: ultrasonic handoff front=%.1f cm reached water threshold <=%d cm"
    WATER_HANDOFF_NO_READING = "Auto: ultrasonic handoff has no front reading; holding still"
    WATER_HANDOFF_EXPIRED = "Auto: ultrasonic watering handoff expired; resuming normal target and obstacle logic"
    ...
    POST_WATER_REVERSING = "Auto: reversing for %.1fs after watering, then looking for %s"
    POST_WATER_SETTLE = "Auto: watered plant recorded; settling in place for %.0fs before resuming"
    RETREAT_NEW_PLANT = "Auto: new unwatered plant spotted during retreat (id=%s)"
    RETREAT_REID_VIEW = "Auto: +1 retreat ReID view (id=%s)"
    SCAN_CONTINUOUS = "Auto: continuous scan — rotating %s at speed %.2f until a target appears"
    SCAN_TARGET_ACQUIRED = "Auto: target acquired after scan; stopping to re-evaluate"
    OBSTACLE_AVOIDANCE = "Auto: obstacle detected before YOLO watering handoff; scan chose %s"
    OBSTACLE_BACKUP = "Auto: obstacle avoidance reverse for %.1fs"
    OBSTACLE_SWING = "Auto: obstacle avoidance swing %s for %.1fs"
    OBSTACLE_FORWARD = "Auto: obstacle avoidance forward for %.1fs"
    OBSTACLE_RESTORE = "Auto: obstacle avoidance undo swing %s for %.1fs"
